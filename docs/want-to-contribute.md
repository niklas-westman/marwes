...
3. [Adapter Architecture](./reference/adapter-architecture.md) — understand the cross-framework adapter ruleset.
4. [Repo Map](./reference/repo-map.md) — know which files and checks move together.
...
- `packages/react`, `packages/vue`, and `packages/svelte` adapt the shared recipe into framework APIs.
...
4. **Update React, Vue, and Svelte adapters together** so framework behavior does not drift.
5. **Update Storybook in React, Vue, and Svelte** so examples stay comparable.
...
- Keep React, Vue, and Svelte parity visible.
...
- Do not put core logic in React, Vue, or Svelte adapters.
