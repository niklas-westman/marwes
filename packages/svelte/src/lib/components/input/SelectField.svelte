<script lang="ts">
  ...
  const invalid = $derived(hasError || false);
  ...
  const wrapperClass = $derived(
    mergeClass("mw-input-field", disabled && "mw-input-field--disabled", invalid && "mw-input-field--invalid", className)
  );
</script>
...
<div class={wrapperClass}>
  ...
  <div class="mw-input-field__input-wrapper">
    <Select {...select} id={fieldId} describedBy={a11yIds.describedBy ?? undefined} bind:value />
  </div>
