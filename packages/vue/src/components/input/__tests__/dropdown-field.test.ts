/**
 * Vue adapter: Tests the Dropdown Field molecule — wires the shared contract for label,
...
function renderWithProvider(props: DropdownFieldProps) {
  render(
    defineComponent({
      setup() {
        return () => h(MarwesProvider, null, { default: () => h(DropdownField, props) })
      },
    }),
  )
}

runDropdownFieldContract("vue", {
