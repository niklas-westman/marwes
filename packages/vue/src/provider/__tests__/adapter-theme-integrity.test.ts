/**
 * Vue adapter: Static analysis guard: scans adapter source files for hardcoded color
...
function listSourceFiles(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (entry.isFile() && /\.ts$/.test(path)) return [path]

    return []
  })
}

describe("Vue adapter theme integrity", () => {
