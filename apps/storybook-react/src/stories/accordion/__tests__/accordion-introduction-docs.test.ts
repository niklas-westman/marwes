/**
 * React Accordion introduction docs guard — verifies that the
 * Introduction.mdx file documents all expected sections and component references.
 */
import { readFileSync } from "node:fs"
import path from "node:path"
import { describe, expect, it } from "vitest"

describe("React accordion introduction docs", () => {
  it("documents atom, molecule, and purpose components", () => {
    const introPath = path.resolve(__dirname, "../Introduction.mdx")
    const introDoc = readFileSync(introPath, "utf8")

    expect(introDoc).toContain("Accordion (Atom)")
    expect(introDoc).toContain("AccordionField (Molecule)")

    const variants = ["FAQAccordion", "SettingsAccordion", "SectionsAccordion"]

    for (const variantName of variants) {
      expect(introDoc).toContain(variantName)
    }
  })
})
