...
1. **Automatic semantic attributes** - Correct `type`, `inputMode`, `autoComplete` set automatically
2. **Built-in functionality** - Password toggle, search clear button, etc. included
3. **Mobile-optimized** - Proper keyboard layouts on mobile devices
4. **AI-friendly** - `data-purpose` attributes for AI parsing and understanding
5. **Less boilerplate** - No need to remember which attributes to set
6. **Self-documenting** - Code intent is clear from component name

## Field Comparison

| Field | Type | inputMode | autoComplete | Special Features |
|-------|------|-----------|--------------|------------------|
| **SearchField** | `search` | `search` | - | Clear button (X icon) when has value |
| **PasswordField** | `password` | - | `current-password` or `new-password` | Show/hide toggle (eye icon) |
| **EmailField** | `email` | `email` | `email` | Email validation, @ keyboard |
| **PhoneField** | `tel` | `tel` | `tel` | Phone keyboard, accepts any format |
| **URLField** | `url` | `url` | `url` | URL validation, .com keyboard |
| **CurrencyField** | `text` | `decimal` | - | Decimal keyboard, currency label |
