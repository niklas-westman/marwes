/**
 * Vue Input story taxonomy guard — verifies that story files
...
describe("Vue input story taxonomy", () => {
  ...
  it("uses Atom title for RichText story", () => {
    const richTextStory = readStoryFile("rich-text.stories.ts")
  ...
  it("uses Molecule title for RichTextField story", () => {
    const richTextFieldStory = readStoryFile("rich-text-field.stories.ts")
