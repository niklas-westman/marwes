/**
 * Svelte Tab story taxonomy guard — verifies that story files
...
describe("Svelte tab story taxonomy", () => {
  ...
  it("keeps multi-state atom stories wired to the Svelte showcase", () => {
    const atomStory = readStoryFile("tab.stories.ts")

    expect(atomStory).toContain("TabShowcase")
    expect(atomStory).toContain('showcase: "states"')
    expect(atomStory).toContain('showcase: "bar"')
  })
})
