/**
 * Svelte Slider story taxonomy guard — verifies that story files
...
describe("Svelte slider story taxonomy", () => {
  it("keeps atom, molecule, and purpose stories under the Slider taxonomy", () => {
    const atomStory = readFileSync(path.join(storiesDir, "slider.stories.ts"), "utf8")
    const moleculeStory = readFileSync(path.join(storiesDir, "slider-field.stories.ts"), "utf8")
    const volumeStory = readFileSync(path.join(storiesDir, "volume-slider.stories.ts"), "utf8")
    const brightnessStory = readFileSync(
      path.join(storiesDir, "brightness-slider.stories.ts"),
      "utf8",
    )
    const radiusStory = readFileSync(path.join(storiesDir, "radius-slider.stories.ts"), "utf8")
